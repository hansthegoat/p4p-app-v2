...
import {
  ...
  Layers, ListChecks, Building2, UserCog,
} from "lucide-react";
...
function KPIFrameworkPage() {
  const { getTemplate, saveTemplate, getAllTemplates } = useP4P();
              ...
              <div className="flex items-center gap-2 h-10 px-3 rounded-md border border-border bg-muted/30 text-sm">
                {template ? (
                  <>
                    <Badge
                      variant="outline"
                      className={`bg-${weightStatus.color}-500/10 text-${weightStatus.color}-700 dark:text-${weightStatus.color}-400 border-${weightStatus.color}-500/30 gap-1`}
                    >
                  ...
                  <span className="text-muted-foreground text-xs">No template selected</span>
                )}
              </div>
            </div>
          </div>
                  ...
                  <motion.div key={cat.id} variants={fadeUp} layout>
                    <Card className="overflow-hidden">
                      ...
                      <div className="p-4 space-y-3">
                        {cat.kpis.length === 0 ? (
                          <div className="text-sm text-muted-foreground text-center py-6 border border-dashed border-border/50 rounded-lg">
