...
export function TourReplayButton({ role }: TourReplayButtonProps) {
  ...
  const startTour = (tour: Tour) => {
    ...
    const runWithFallback = (attempt: number) => {
      const available = filterAvailableSteps(tour.steps);
      if (available.length === 0) {
        // Navigate to the page first if this is a page tour
        if (attempt === 0 && tour.key.startsWith("page_")) {
          const route = "/" + tour.key.replace("page_", "");
          window.location.href = `${window.location.origin}${import.meta.env.BASE_URL}${route.replace(/^\//, "")}`;
          return;
        }
        return;
      }
