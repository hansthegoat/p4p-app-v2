...
import { i as init } from "../_libs/sentry__react.mjs";
...
const appCss = "/assets/styles-BG-pB-n0.css";
...
function ctaButton(label, url, color = "#18181b") {
...
function noticeCard(text, color = "blue") {
  const colors = {
...
function P4PProvider({ children }) {
  ...
  const [isCloudSynced, setIsCloudSynced] = reactExports.useState(false);
  ...
  reactExports.useEffect(() => {
    ...
    const refetchFromCloud = async () => {
      setIsSyncing(true);
      try {
        const [cloudEmployees, cloudTemplates, cloudMonthly, cloudAppraisals, cloudKpiUpdates, cloudSettings] = await Promise.all([
          fetchAllEmployees(),
          fetchAllTemplates(),
          fetchAllMonthly(),
          fetchAllAppraisals(),
  ...
  const setGlobals = reactExports.useCallback(
    (g) => setState((s) => ({ ...s, globals: { ...s.globals, ...g } })),
    []
  );
  const setGrades = reactExports.useCallback(
    (grades) => setState((s) => ({ ...s, grades })),
    []
  );
  const resetGrades = reactExports.useCallback(
    () => setState((s) => ({ ...s, grades: DEFAULT_GRADES })),
    []
  );
  const upsertEmployee = reactExports.useCallback((e) => {
    setState((s) => {
  ...
  const removeEmployee = reactExports.useCallback((id2) => {
  ...
  const clearEmployees = reactExports.useCallback(() => {
    setState((s) => ({ ...s, employees: [] }));
  }, []);
  const loadDemo = reactExports.useCallback(() => {
    setState({
      globals: DEFAULT_GLOBALS,
      grades: DEFAULT_GRADES,
      employees: DEMO_EMPLOYEES,
  ...
  const saveMonthlySnapshot = reactExports.useCallback(
  ...
  const getMonthlyHistory = reactExports.useCallback(
  ...
  const getPerformanceTrend = reactExports.useCallback(
    (employeeId) => {
  ...
  const getAllTrends = reactExports.useCallback(() => {
    const trends = [];
  ...
  const deleteMonthlyData = reactExports.useCallback(
    (employeeId, year, month) => {
      setState((s) => ({
        ...s,
  ...
  const getMonthData = reactExports.useCallback(
    (year, month) => state.monthlyData.filter((d) => d.year === year && d.month === month),
    [state.monthlyData]
  );
  const getMonthlyStats = reactExports.useCallback(() => {
  ...
  const detectTriggers = reactExports.useCallback(() => {
    const triggers = [];
    for (const emp of state.employees) {
      if (emp.isAdjunct) continue;
  ...
  const getTriggersForEmployee = reactExports.useCallback(
    (employeeId) => {
  ...
  const saveTemplate = reactExports.useCallback((template) => {
  ...
  const getTemplate = reactExports.useCallback(
    (department, role) => {
      return state.kpiTemplates[`${department}-${role}`];
    },
    [state.kpiTemplates]
  );
  const getAllTemplates = reactExports.useCallback(() => state.kpiTemplates, [state.kpiTemplates]);
  const applyTemplateToEmployees = reactExports.useCallback(
  ...
  const sendAppraisalNotification = reactExports.useCallback(
  ...
  const submitAppraisal = reactExports.useCallback(
  ...
  const saveKPIProof = reactExports.useCallback(
  ...
  const saveKPIComment = reactExports.useCallback(
  ...
  const triggerAfterApproval = reactExports.useCallback(
    (employeeId) => {
  ...
  const approveAppraisal = reactExports.useCallback(
  ...
  const rejectAppraisal = reactExports.useCallback(
  ...
  const requestChanges = reactExports.useCallback(
    (id2, reviewerId, reviewerName, reason) => {
  ...
  const getEmployeeAppraisals = reactExports.useCallback(
    (employeeId) => state.appraisals.filter((a) => a.employeeId === employeeId).sort(
      (a, b) => new Date(b.submittedAt).getTime() - new Date(a.submittedAt).getTime()
    ),
    [state.appraisals]
  );
  const getPendingAppraisals = reactExports.useCallback(
    () => state.appraisals.filter((a) => a.status === "pending").sort(
      (a, b) => new Date(b.submittedAt).getTime() - new Date(a.submittedAt).getTime()
    ),
    [state.appraisals]
  );
  const addAppraisalComment = reactExports.useCallback(
  ...
  const getNotifications = reactExports.useCallback(
  ...
  const previewTemplateDiff = reactExports.useCallback(
    (department, role, template) => {
  ...
  const pushTemplateToEmployees = reactExports.useCallback(
  ...
  const getKpiUpdateRequests = reactExports.useCallback(
    (employeeId) => state.kpiUpdateRequests.filter((r) => r.employeeId === employeeId).sort(
      (a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
    ),
    [state.kpiUpdateRequests]
  );
  const getUnacknowledgedKpiUpdates = reactExports.useCallback(
    () => state.kpiUpdateRequests.filter((r) => r.status === "unacknowledged"),
    [state.kpiUpdateRequests]
  );
  const acknowledgeKpiUpdate = reactExports.useCallback(
    async (id2) => {
  ...
  const commentKpiUpdate = reactExports.useCallback(
    async (id2, comment) => {
      const req = state.kpiUpdateRequests.find((r) => r.id === id2);
      if (!req) return;
  ...
  const setBonusRevealed = reactExports.useCallback(async (value2) => {
    setState((s) => ({ ...s, bonusRevealed: value2 }));
    try {
      await upsertAppSetting("bonus_revealed", value2);
    } catch (err) {
  ...
  const syncToCloud = reactExports.useCallback(async () => {
    setIsSyncing(true);
    try {
      await bulkUpsertEmployees(state.employees);
      for (const key in state.kpiTemplates) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(C.Provider, { value, children });
}
function useP4P() {
  const v = reactExports.useContext(C);
...
const UserProvider = ({ children }) => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(UserContext.Provider, { value: { user, employee, role, loading }, children });
};
...
function SplashScreen() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      className: `fixed inset-0 z-[200] flex flex-col items-center justify-center transition-opacity ${fading ? "opacity-0 pointer-events-none" : "opacity-100"}`,
      style: {
      ...
      children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mt-12 w-44 h-[2px] bg-slate-100/80 rounded-full overflow-hidden", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
          "div",
          {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "p",
          {
            ...
            style: { opacity: 0 },
            children: "Precision Performance"
          }
        )
      ]
    }
...
function RootComponent() {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(QueryClientProvider, { client: queryClient, children: /* @__PURE__ */ jsxRuntimeExports.jsx(ThemeProvider, { defaultTheme: "light", storageKey: "p4p-theme", children: /* @__PURE__ */ jsxRuntimeExports.jsx(P4PProvider, { children: /* @__PURE__ */ jsxRuntimeExports.jsxs(UserProvider, { children: [
...
const $$splitComponentImporter$l = () => import("./verify-otp-DKQVdjwd.mjs");
const Route$m = createFileRoute("/verify-otp")({
  component: lazyRouteComponent($$splitComponentImporter$l, "component")
});
const $$splitComponentImporter$k = () => import("./reset-password-_9eRlcsB.mjs");
const Route$l = createFileRoute("/reset-password")({
  component: lazyRouteComponent($$splitComponentImporter$k, "component")
});
const $$splitComponentImporter$j = () => import("./forgot-password-Cx8f4u-f.mjs");
const Route$k = createFileRoute("/forgot-password")({
  component: lazyRouteComponent($$splitComponentImporter$j, "component")
});
const $$splitComponentImporter$i = () => import("../_app-CU3hKCw2.mjs");
...
const $$splitComponentImporter$h = () => import("./route-CQVltfbh.mjs");
...
const $$splitComponentImporter$g = () => import("./register-DchrstiM.mjs");
const searchSchema = objectType({
  email: stringType().optional()
});
const Route$g = createFileRoute("/_auth/register")({
  validateSearch: searchSchema,
  component: lazyRouteComponent($$splitComponentImporter$g, "component")
});
const $$splitComponentImporter$f = () => import("./login-BWGyUGa2.mjs");
const Route$f = createFileRoute("/_auth/login")({
  component: lazyRouteComponent($$splitComponentImporter$f, "component")
});
const $$splitComponentImporter$e = () => import("../_app.trace-CWY3iX6T.mjs");
const Route$e = createFileRoute("/_app/trace")({
  component: lazyRouteComponent($$splitComponentImporter$e, "component")
});
const $$splitComponentImporter$d = () => import("../_app.supervisors-DLKGuTWZ.mjs");
const Route$d = createFileRoute("/_app/supervisors")({
  component: lazyRouteComponent($$splitComponentImporter$d, "component")
});
const $$splitComponentImporter$c = () => import("../_app.profile-9lIMf9TX.mjs");
const Route$c = createFileRoute("/_app/profile")({
  component: lazyRouteComponent($$splitComponentImporter$c, "component")
});
const $$splitComponentImporter$b = () => import("../_app.my-calculation-Dxm69R16.mjs");
const Route$b = createFileRoute("/_app/my-calculation")({
  component: lazyRouteComponent($$splitComponentImporter$b, "component")
});
const $$splitComponentImporter$a = () => import("../_app.monthly-C3Le9bmw.mjs");
const Route$a = createFileRoute("/_app/monthly")({
  component: lazyRouteComponent($$splitComponentImporter$a, "component")
});
const $$splitComponentImporter$9 = () => import("../_app.kpi-updates-DJJJyiMB.mjs");
const Route$9 = createFileRoute("/_app/kpi-updates")({
  component: lazyRouteComponent($$splitComponentImporter$9, "component")
});
const $$splitComponentImporter$8 = () => import("../_app.kpi-framework-DTqH4X8L.mjs");
const Route$8 = createFileRoute("/_app/kpi-framework")({
  component: lazyRouteComponent($$splitComponentImporter$8, "component")
});
const $$splitComponentImporter$7 = () => import("../_app.grades-TuAJUqU5.mjs");
const Route$7 = createFileRoute("/_app/grades")({
  component: lazyRouteComponent($$splitComponentImporter$7, "component")
});
const $$splitComponentImporter$6 = () => import("../_app.employees-DPq-hTkv.mjs");
const Route$6 = createFileRoute("/_app/employees")({
  component: lazyRouteComponent($$splitComponentImporter$6, "component")
});
const $$splitComponentImporter$5 = () => import("../_app.employee-B2CWvYQk.mjs");
const Route$5 = createFileRoute("/_app/employee")({
  component: lazyRouteComponent($$splitComponentImporter$5, "component")
});
const $$splitComponentImporter$4 = () => import("../_app.dashboard-DHGqLE_-.mjs");
const Route$4 = createFileRoute("/_app/dashboard")({
  component: lazyRouteComponent($$splitComponentImporter$4, "component")
});
const $$splitComponentImporter$3 = () => import("../_app.change-password-Ckaz-pku.mjs");
const Route$3 = createFileRoute("/_app/change-password")({
  component: lazyRouteComponent($$splitComponentImporter$3, "component")
});
const $$splitComponentImporter$2 = () => import("../_app.audit-log-BR1dOH9n.mjs");
const Route$2 = createFileRoute("/_app/audit-log")({
  component: lazyRouteComponent($$splitComponentImporter$2, "component")
});
const $$splitComponentImporter$1 = () => import("../_app.appraisals-review-CXtU6F1M.mjs");
const Route$1 = createFileRoute("/_app/appraisals-review")({
  component: lazyRouteComponent($$splitComponentImporter$1, "component")
});
const $$splitComponentImporter = () => import("../_app.appraisals-Be7-dgRI.mjs");
const Route = createFileRoute("/_app/appraisals")({
  component: lazyRouteComponent($$splitComponentImporter, "component")
});
const VerifyOtpRoute = Route$m.update({
  id: "/verify-otp",
  path: "/verify-otp",
...
const ForgotPasswordRoute = Route$k.update({
  id: "/forgot-password",
...
const AuthRouteRoute = Route$i.update({
  id: "/_auth",
  getParentRoute: () => Route$n
});
...
const AuthRegisterRoute = Route$g.update({
  id: "/register",
  path: "/register",
  getParentRoute: () => AuthRouteRoute
});
const AuthLoginRoute = Route$f.update({
  id: "/login",
  path: "/login",
  getParentRoute: () => AuthRouteRoute
});
const AppTraceRoute = Route$e.update({
  id: "/trace",
  path: "/trace",
  getParentRoute: () => AppRoute
});
const AppSupervisorsRoute = Route$d.update({
  id: "/supervisors",
  path: "/supervisors",
  getParentRoute: () => AppRoute
});
